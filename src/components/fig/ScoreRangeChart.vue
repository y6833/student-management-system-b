...
<template>
  <div>
    <div id="ScoreRangeChart" style="width: 1150px; height: 300px"></div>
  </div>
</template>
...
import {
  getAbscissa,
  getScoreListByExamAndGradeAndSubject,
} from "@/api/scores";
import * as echarts from "echarts";
export default {
  name: "ScoreRangeChart",
  props: ["AveTableData", "choiceSubject", "majorValue"],
  data() {
    return {
      maxnum: 100,
      interval: 50,
  ...
  methods: {
    ...
    async getScoreListByExamAndGradeAndSubject() {
      const params = {
        examValue: this.AveTableData[0].examName,
        gradeValue: this.AveTableData[0].grade,
        majorValue: this.majorValue,
        choiceSubject: this.choiceSubject,
      };
      const res = await getScoreListByExamAndGradeAndSubject(params);
      if (res.code == 200) {
    ...
    async getAbscissa() {
      ...
      const res = await getAbscissa(params);
      if (res.code == 200) {
